...
const PlayerQuizPage = () => {
    ...
    const sendAnswer = async (gameMode) => {
        ...
        const response = await fetch(`http://localhost:8080/quiz/send/answer/${gameMode}?quizId=${quizId}&answer=${answer.current}&roomId=R${roomId.current}&playerName=${playerName.current}`, {
            method: "GET",
            headers: {
    ...
    const handleSkip = async () => {
        const response = await fetch(`http://localhost:8080/quiz/send/skip?roomId=R${roomId.current}&playerName=${playerName.current}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
            },
