...
const PoseRoundResult = ({roomId}) => {
    ...
    const getPoseScore = async () => {
        try {
            const response = await fetch(`http://localhost:8080/quiz/player/score?room_id=R${roomId}&player_name=${nextPlayer}`);
    ...
    const fetchImage = async () => {
        try {
            const response = await fetch(`http://localhost:8080/quiz/pose/image/get?roomId=R${roomId}&playerName=${nextPlayer}`);
